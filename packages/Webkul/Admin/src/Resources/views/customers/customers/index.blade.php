<x-admin::layouts>
    ...
    <x-admin::datagrid src="{{ route('admin.customers.customers.index') }}" ref="customer_data" :isMultiRow="true">
        @php
            $hasPermission =
                bouncer()->hasPermission('customers.customers.mass-update') ||
                bouncer()->hasPermission('customers.customers.mass-delete');
        @endphp
        ...
        <template #body="{ columns, records, setCurrentSelectionMode, applied, isLoading }">
            <template v-if="! isLoading">
                <div class="row grid grid-cols-[minmax(150px,_2fr)_1fr_1fr] px-[16px] py-[10px] border-b-[1px] dark:border-gray-800 transition-all hover:bg-gray-50 dark:hover:bg-gray-950"
                    v-for="record in records">
                    <div class="flex gap-[10px]">
                        @if ($hasPermission)
                            <input type="checkbox" :name="`mass_action_select_record_${record.customer_id}`"
                                :id="`mass_action_select_record_${record.customer_id}`" :value="record.customer_id"
                        ...
                        <div class="flex flex-col gap-[6px]">
                            ...
                            <p class="text-gray-600 dark:text-gray-300" v-text="record.email">
                            </p>

                            <p class="text-gray-600 dark:text-gray-300" v-text="record.phone ?? 'N/A'">
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-[6px]">
                        ...
                        <p class="text-gray-600 dark:text-gray-300" v-text="record.gender ?? 'N/A'">
                        </p>
                    ...
                    <div class="flex gap-x-[16px] justify-between items-center">
                        ...
                        <div class="flex items-center">
                            <form
                                :action=`{{ route('admin.customers.customers.change_status', '') }}/${record.customer_id}`
                                method="POST">
                                @csrf
                                <button
                                    class="text-[16px] ltr:ml-[4px] rtl:mr-[4px] p-[6px] cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-800 hover:rounded-[6px]"
                                    type="submit">
                                    تجميد \ فك تجميد
                                    </a>
                            </form>
